...
@Component({
  templateUrl: "./add-doctorManagement.component.html"
})
export class AddDoctorComponent implements OnInit {
  ...
  constructor(
    public doctorManagementFactory: DoctorManagementService,
  ...
  ngOnInit() {
    ...
    this.doctorManagementFactory.getPermissionList().subscribe(response => {
  ...
  addDoctorFormSubmit(): void {
    ...
    if (this.addDoctorForm.valid) {
      let formData = _.clone(this.addDoctorForm.value);
      formData.createdBy = this.adminDetails._id;

      if (
        this.adminDetails.userType == "superAdmin" ||
        this.adminDetails.userType == "subAdmin"
      ) {
        formData.userType = "superDoctor";
      } else {
        formData.userType = "subDoctor";
      }

      if (formData.permissions.length) {
        formData.permissions = _.map(
          this.addDoctorForm.value.permissions,
          "_id"
        );
      } else {
      ...
      this.doctorManagementFactory.addNewDoctor(formData).subscribe(
        response => {
          let finalResult = JSON.parse(JSON.stringify(response));
          if (finalResult.status == 200) {
