...
@Component({
  templateUrl: "./add-labAssistantManagement.component.html"
})
export class AddLabAssistantComponent implements OnInit {
  addLabAssistantForm: FormGroup;
  ...
  constructor(
    public labAssistantManagementFactory: LabAssistantManagementService,
  ...
  ngOnInit() {
    this.addLabAssistantForm = new FormGroup({
  ...
  addLabAssistantFormSubmit(): void {
    this.submitted = true;
    if (this.addLabAssistantForm.valid) {
      let formData = _.clone(this.addLabAssistantForm.value)
      ...
      if (this.adminDetails.userType == 'superAdmin' || this.adminDetails.userType == 'subAdmin') {
        formData.userType = 'superLabAssistant';
      } else {
        formData.userType = 'subLabAssistant';
      }
      if (formData.permissions.length) {
        formData.permissions = _.map(this.addLabAssistantForm.value.permissions, '_id');
      } else {
        formData.permissions = [];
      }

      this.labAssistantManagementFactory
        .addNewLabAssistant(formData)
        .subscribe(
          response => {
            let finalResult = JSON.parse(JSON.stringify(response));
            if (finalResult.status == 200) {
